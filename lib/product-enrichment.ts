...
function buildProductSummary(product: Product): string {
  ...
  parts.push(`a ${product.category === "foldable-smartphones" ? "foldable " : ""}smartphone`)
...
function buildBestFor(product: Product): string[] {
  ...
  if (product.category === "foldable-smartphones") best.push("Multitaskers and power users")
...
function buildAITags(product: Product): string[] {
  ...
  if (product.category === "foldable-smartphones") {
    tags.push("foldable")
  } else {
